...
class GoogleMapPlotter(object):
    ...
    def marker(self, lat, lng, rotation, color='#FF0000', c=None, title="no implementation"):
        if c:
            color = c
        color = self.color_dict.get(color, color)
        color = self.html_color_codes.get(color, color)
        self.points.append((lat, lng, rotation, color, title))
    ...
    def write_point(self, f, lat, lon, rotation, color, title):
        f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n' %
                (lat, lon))
        f.write("var image = {")
        f.write("path: google.maps.SymbolPath.FORWARD_CLOSED_ARROW,")
        f.write("scale: 6,")
        f.write("fillColor: '{}',".format(color))
        f.write("fillOpacity: 1,")
        f.write("strokeWeight: 1,")
        f.write("rotation: {}".format(rotation))
        f.write("};")
